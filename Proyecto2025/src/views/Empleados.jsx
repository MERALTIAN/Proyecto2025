...
const Empleados = () => {
  ...
  const cargarEmpleados = async () => {
    try {
      ...
      if (error) {
        console.error('Supabase empleados error:', error);
  ...
  const agregarEmpleado = async () => {
    const nombre = nuevoEmpleado.nombre_empleado.trim();
    const apellido = nuevoEmpleado.apellido_empleado.trim();
    const email = nuevoEmpleado.email.trim().toLowerCase();
    const password = nuevoEmpleado.password;
    const tipo = nuevoEmpleado.tipo_empleado;
    ...
    if (!emailRegex.test(email)) {
      setToast({ mostrar: true, mensaje: "Ingrese un correo electrónico válido", tipo: "advertencia" });
      return;
    }

    if (password.length < 6) {
      setToast({ mostrar: true, mensaje: "La contraseña debe tener al menos 6 caracteres", tipo: "advertencia" });
      return;
    }
    ...
    try {
      ...
      const { error: dbError } = await supabase.from("empleados").insert([{
        ...
        celular: nuevoEmpleado.celular.trim(),
        pin: nuevoEmpleado.pin.trim(),
        email,
        tipo_empleado: tipo,
      }]);
      ...
      setToast({
        mostrar: true,
        mensaje: `Empleado ${nombre} registrado correctamente`,
        tipo: "exito"
      });
    } catch (err) {
      console.error(err);
  ...
  return (
    <Container className="mt-5 pt-3">
      ...
      {!cargando && empleadosFiltrados.length === 0 && !textoBusqueda.trim() && (
        <Row className="text-center my-5">
          <Col>
            <Alert variant="warning">
              No hay empleados registrados todavía. Usa el botón "Nuevo Empleado" para agregar uno.
            </Alert>
          </Col>
        </Row>
      )}
