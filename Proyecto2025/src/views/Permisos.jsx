...
const Permisos = () => {
  ...
  const cargarRoles = async () => {
    try {
      ...
      if (error) {
        console.error('Supabase permisos error:', error);
        throw error;
      }
  ...
  return (
    <Container className="mt-5 pt-3">
      ...
      {!cargando && rolesFiltrados.length === 0 && !textoBusqueda.trim() && (
        <Row className="text-center my-5">
          <Col>
            <Alert variant="warning">
              No hay roles configurados. Ejecuta el script SQL para crear los permisos.
            </Alert>
          </Col>
        </Row>
      )}
