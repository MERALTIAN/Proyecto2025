...
import TarjetaCatalogo from "../catalogo/TarjetaCatalago";
...
const Catalogo = () => {
  ...
  const productosFiltrados = useMemo(() => {
    ...
    if (categoriaSeleccionada !== "todas") {
      filtrados = filtrados.filter((prod) => {
        const categoriaId = prod.id_Categoria ?? prod.categoria_producto;
  ...
  const obtenerNombreCategoria = (idCategoria) => {
    const cat = categorias.find(
      (c) => (c.id_Categoria ?? c.id_categoria) === idCategoria
    );
        ...
        <Col md={4} lg={3} className="mb-2">
          <Form.Group controlId="filtroCategoria">
            ...
            >
              ...
              {categorias.map((cat) => {
                const categoriaId = cat.id_Categoria ?? cat.id_categoria;
                return (
        ...
        <Row className="g-3">
          {productosFiltrados.map((producto) => (
            <Col
              ...
              md={4}
              lg={3}
              key={producto.id_Producto ?? producto.id_producto}
            >
              <TarjetaCatalogo
                producto={producto}
                categoriaNombre={obtenerNombreCategoria(
                  producto.id_Categoria ?? producto.categoria_producto
                )}
              />
            </Col>
          ))}
